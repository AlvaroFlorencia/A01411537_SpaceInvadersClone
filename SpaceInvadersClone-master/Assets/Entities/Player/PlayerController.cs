...
public class PlayerController : MonoBehaviour {
	...
	void Firer () {
		...
		//	them.
		Vector3 right = new Vector3(0.35f, 0, 0);
		GameObject beamr = Instantiate (projectile, transform.position + right, Quaternion.identity) as GameObject;
		rgb = beamr.GetComponent<Rigidbody2D> ();
	...
	void Firel () {
		...
		Vector3 left = new Vector3(-0.35f, 0, 0);
		GameObject beaml = Instantiate (projectile, transform.position + left, Quaternion.identity) as GameObject;
		rgb = beaml.GetComponent<Rigidbody2D> ();
		rgb.velocity = new Vector3 (0, projectileSpeed, 0);
		//	We play the fireSound Clip
		AudioSource.PlayClipAtPoint (fireSound, transform.position);
	}
	...
	void Update () {
		...
		if (Input.GetKeyDown (KeyCode.D)) {
			InvokeRepeating ("Firer", 0.000001f, firingRate);
			animator.SetTrigger ("Disparador");  // If the player press "A" the animation is shown
		}
		...
		if (Input.GetKeyDown (KeyCode.A)) {
			InvokeRepeating ("Firel", 0.000001f, firingRate);
			animator.SetTrigger ("Disparador"); // If the player press "D" the animation is shown
		}
	...
	void Die() {
		...
		levelManager.LoadLevel ("Lost Screen");  //If the player dies,then the the scene(level) "lost Screen" is loaded
		Destroy (gameObject);
	}
}
